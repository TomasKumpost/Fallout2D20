...
def start(screen):
    ...
    custom_item = [Box(150, 50, cust_x, cust_y, "light gray", "Custom item:"),
                   ...
                   Box(80, 50, cust_x + 520, cust_y, "light gray", "cost"),
...
def return_values(catalog, custom_item):
    ...
    if custom_item[2].text != "":
        if custom_item[-1].text != "":
            return_list.append((custom_item[2].text, int(custom_item[-1].text)))
        else:
            return_list.append((custom_item[2].text, 1))

    for category, items in catalog:
        for item in items:
            if item.click:
                return_list.append((item.text, item.cost))
    return return_list
...
def parse_items():
    ...
    for category in catalog:
        ...
        items = [ItemButton(300, 75, 0, 0, "light gray", "dark gray", int(item[1]),
                            item[0], True, 25)
                 for item in [(item.attrib.get("name"), item.text) for item in category]]
